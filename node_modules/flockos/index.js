...
var methodsBaseUrl = 'https://api.flock-staging.co/v1';
...
reply.listener = express.Router();
reply.listener.use(require('body-parser').json());
reply.listener.use(function (req, res, next) {
    console.log('received request: ', req.method, req.url, req.headers);
    console.log('received event: %j', req.body);
	
	var event = {};
	
	event.messageUid = req.query.threadMessageUid;
	event.replyMessage = req.query.replyMessage;
	event.userId = req.query.userId;
	event.userName = req.query.userName;
	event.name = req.query.eventName;
	
	console.log('received event: %j', event);
	
    var responded = false;
    reply.listeners(event.name).forEach(function (listener) {
        var body = listener(event);
...
createMethod('chat.addAttachments');
